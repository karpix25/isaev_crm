    ...
    async def build_context(
        ...
        if followup_pause and lead_status in {LeadStatus.FOLLOW_UP.value, LeadStatus.KEYS_PENDING.value}:
            ...
            response_policy = [
                ...
                "Не делать вид, что ИИ прочитал файл сметы.",
                "Если клиент спрашивает про цифры или пункты сметы, предложить передать вопрос менеджеру или сметчику.",
...
- Не повторяй однотипные фразы из прошлых сообщений: "стоимость зависит", "чтобы точнее", "подскажите, пожалуйста".
- Редактируй ответ перед отправкой: убери вводные слова, пассивные обороты и общие фразы, если они не помогают клиенту.
- Предпочитай конкретные действия: "пришлите проект", "выберите время", "напишите адрес" вместо абстрактного "уточним детали".
- По умолчанию отвечай 1–4 короткими предложениями.
- Если клиент просит только вилку цен, дай preliminary_estimate и одну короткую оговорку. Не убеждай в замере длинно.

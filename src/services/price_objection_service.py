...
class PriceObjectionService:
    ...
    def build_reply(self, *, lead, text: str) -> PriceObjectionReply:
        ...
        if fit in {"inside_range", "near_low"}:
            ...
            text_reply = (
                f"Понял, ориентир {budget_text} — это нормальная рамка. Мы как раз рядом: {estimate}. "
                "Чтобы понять, реально ли удержаться ближе к нижней границе, лучше не гадать по анкете, а посмотреть объект. "
                "Замер бесплатный и ни к чему не обязывает. Давайте предложу пару ближайших окон, а вы выберете удобное?"
            )

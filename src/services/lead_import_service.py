...
class LeadImportService:
    ...
    def _parse_status(value: Optional[str]) -> Optional[LeadStatus]:
        ...
        status_rules: list[tuple[list[str], LeadStatus]] = [
            ...
            (["quiz", "квиз", "анкета"], LeadStatus.QUIZ_COMPLETED),
            (["messenger", "мессендж", "ждем сообщение", "ждём сообщение"], LeadStatus.MESSENGER_PENDING),
            (["design pending", "ждем проект", "ждём проект", "ждем дизайн", "ждём дизайн"], LeadStatus.DESIGN_PENDING),
            (["design review", "разбор проект", "проект на разбор"], LeadStatus.DESIGN_REVIEW),
            (["consult", "консультац"], LeadStatus.CONSULTING),
            (["measurement pending", "ждем запись", "ждём запись", "запись на замер"], LeadStatus.MEASUREMENT_PENDING),
            (["measurement booked", "замер назнач", "назначен замер"], LeadStatus.MEASUREMENT_BOOKED),
            (["measurement done", "замер провед", "проведен замер", "проведён замер"], LeadStatus.MEASUREMENT_DONE),
            (["estimate preparing", "смета готов", "готовится смета"], LeadStatus.ESTIMATE_PREPARING),
            (["estimate review", "смета на провер", "проверка сметы"], LeadStatus.ESTIMATE_REVIEW),
            (["estimate sent", "смета отправ", "отправлена смета"], LeadStatus.ESTIMATE_SENT),

...
export function Login() {
    ...
    const telegramBotName = String((import.meta as any).env.VITE_TELEGRAM_BOT_NAME || 'isaev_karpix_bot').replace(
        /^@/,
        ''
    )
                    ...
                    <div className="flex flex-col items-center gap-6">
                        ...
                        {loading ? (
                            <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                ...
                                <span>Проверка доступов…</span>
                            </div>
                        ) : (
                            <div ref={containerRef} className="h-[40px] flex items-center justify-center" />
                        )}

                        <p className="text-center text-xs text-muted-foreground leading-relaxed max-w-[320px]">
                            Если вы администратор — добавьте домен сайта в @BotFather через команду /setdomain для вашего бота.
                        </p>
                    </div>
                </div>
            </div>
